...
export async function POST(request:NextRequest){
 try{
  ...
  const ourRateResult=await searchCompetitor(serpApiKey,{name:property.name,url:"",active:true},criteria),results:RateResult[]=[];for(const competitor of competitors)results.push(await searchCompetitor(serpApiKey,competitor,criteria));
