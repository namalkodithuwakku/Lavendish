...
export async function POST(request:NextRequest){
 try{
  ...
  const searchWithFallback=async(competitor:Competitor)=>serpApiKey?searchCompetitor(serpApiKey,competitor,criteria,searchArea):searchXotelo(competitor,criteria);
  ...
  await db.from("intelligence_runs").insert({module:"YIELD",run_type:user.scheduled?"SCHEDULED":"MANUAL",status:"COMPLETED",requested_by:user.id,started_at:new Date().toISOString(),completed_at:new Date().toISOString(),result_summary:{propertyId:property.id,checkIn:criteria.checkIn,checkOut:criteria.checkOut,provider:serpApiKey?"SERPAPI":"XOTELO",competitorCount:competitors.length,verifiedRateCount:verifiedCount,fallbackCount,advisoryOnly:true}});
