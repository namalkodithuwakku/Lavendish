...
async function searchCompetitor(apiKey:string,competitor:Competitor,criteria:Criteria):Promise<RateResult>{
 const nights=stayLength(criteria.checkIn,criteria.checkOut),attemptErrors:string[]=[];
 for(let days=0;days<=7;days++){
  const checkIn=addDays(criteria.checkIn,days),checkOut=addDays(checkIn,nights),params=new URLSearchParams({engine:"google_hotels",q:`${competitor.name} Sri Lanka hotel`,check_in_date:checkIn,check_out_date:checkOut,adults:"2",children:"0",currency:criteria.currency,gl:"lk",hl:"en",api_key:apiKey});
...
export async function POST(request:NextRequest){
 try{
  ...
  const results:RateResult[]=[];for(const competitor of competitors)results.push(await searchCompetitor(serpApiKey,competitor,criteria));
  ...
  const analysis={checkedAt:new Date().toISOString(),summary:`${verified.length} of ${competitors.length} competitor rates verified${fallbackCount?`; ${fallbackCount} found on later dates`:""}.`,suggestedRate,reason:median==null?"No publicly verifiable comparable rates were found.":`Based on the median verified competitor rate${occupancy?` and ${occupancy}% occupancy`:""}.`,guide:failedCount?"Review unavailable hotels using their Google Hotels links; do not treat missing API rates as sold out.":"Verify room, meal-plan, cancellation and tax parity before updating channels.",confidence:verified.length>=4?"HIGH":verified.length>=2?"MEDIUM":"LOW",results};
