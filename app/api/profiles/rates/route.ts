...
You MUST perform fresh web searches for the exact hotel names, dates, guest count, room count, meal plan and requested currency. Search official booking engines, Google Hotels and major OTA pages. Return exactly one results item for every competitor supplied above and keep the same order. A search-result price may be used only when its displayed stay details match the request; otherwise return rate:null with status:"NOT_VERIFIED" and explain which source was checked and why the price could not be verified. Never return an empty results array when competitors were supplied.\n\nThe suggested rate must use the requested currency, our current rate, verified comparable rates and known occupancy. If evidence is insufficient, suggestedRate must be null and the guide must request a manual check. This is advisory only; do not claim any OTA was updated.`;
