...
{"checkedAt":"ISO time","summary":"short operational summary","suggestedRate":number|null,"reason":"concise evidence-based reason","guide":"specific manual rate/channel action","confidence":"HIGH|MEDIUM|LOW","results":[{"hotel":"name","status":"AVAILABLE|FALLBACK_DATE|UNAVAILABLE|NOT_VERIFIED","rate":number|null,"currency":"code","requestedCheckIn":"YYYY-MM-DD","availableCheckIn":"YYYY-MM-DD or blank","availableCheckOut":"YYYY-MM-DD or blank","daysShifted":number,"datesChecked":"short date range","room":"room name or blank","mealPlan":"plan or blank","cancellation":"terms or blank","taxes":"included/excluded/unknown","source":"source name","sourceUrl":"direct public URL","note":"short comparability note"}]}
You MUST perform fresh web searches independently for every competitor using the exact hotel name, guest count, room count, meal plan and requested currency. First search the requested check-in and check-out. If that hotel has no available or verifiable rate, move both dates forward by one day while preserving the stay length, and retry day by day for up to 7 days after the requested check-in. Stop at the first verifiable available rate for that hotel, mark it FALLBACK_DATE, and report availableCheckIn, availableCheckOut and daysShifted. Continue with the next competitor even when one hotel is unavailable or blocked. Search official booking engines, Google Hotels and major OTA pages. Return exactly one results item for every competitor supplied above and keep the same order. For the original dates use status AVAILABLE and daysShifted 0. When all eight arrival dates fail, return rate:null with UNAVAILABLE or NOT_VERIFIED, include the full datesChecked range, and explain which sources were checked. Never return an empty results array when competitors were supplied.\n\nThe suggested rate must use the requested currency, our current rate, verified comparable rates and known occupancy. If evidence is insufficient, suggestedRate must be null and the guide must request a manual check. This is advisory only; do not claim any OTA was updated.`;
  const ai=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:process.env.OPENAI_RATE_MODEL||process.env.OPENAI_PROFILE_MODEL||"gpt-5.4-mini",tools:[{type:"web_search",search_context_size:"high"}],tool_choice:"required",reasoning:{effort:"low"},input:prompt,max_output_tokens:8000})});
