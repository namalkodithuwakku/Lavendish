...
async function requireMaster(request:NextRequest){const authorization=request.headers.get("authorization")??"",token=authorization.startsWith("Bearer ")?authorization.slice(7):"";if(!token)return null;const cronSecrets=[process.env.CRON_SECRET,process.env.YIELD_CRON_SECRET].filter(Boolean);if(cronSecrets.some(secret=>token===secret))return {id:null,scheduled:true};const auth=createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:false,autoRefreshToken:false}});const {data:{user}}=await auth.auth.getUser(token);if(!user)return null;const {data}=await adminClient().from("occupancy_user_access").select("role,active").eq("user_id",user.id).maybeSingle();return data?.active&&data.role==="MASTER_ADMIN"?{id:user.id,scheduled:false}:null}
...
type XoteloResponse={error?:unknown;result?:{rates?:Array<{code?:string;name?:string;rate?:number}>}};

async function searchXotelo(competitor:Competitor,criteria:Criteria):Promise<RateResult>{
 const nights=stayLength(criteria.checkIn,criteria.checkOut),attemptErrors:string[]=[];
 if(!competitor.xoteloHotelKey)return emptyResult(competitor.name,criteria,"Xotelo hotel match is not configured.",competitor.url);
 if(!["USD","GBP","EUR"].includes(criteria.currency))return emptyResult(competitor.name,criteria,`Xotelo does not support ${criteria.currency}.`,competitor.url);
 for(let days=0;days<=7;days++){
  const checkIn=addDays(criteria.checkIn,days),checkOut=addDays(checkIn,nights),params=new URLSearchParams({hotel_key:competitor.xoteloHotelKey,chk_in:checkIn,chk_out:checkOut,rooms:"1",adults:"2",currency:criteria.currency});
  try{
   const response=await fetch(`https://data.xotelo.com/api/rates?${params.toString()}`,{cache:"no-store"}),payload=await response.json() as XoteloResponse;
   if(!response.ok||payload.error){attemptErrors.push(`${checkIn}: ${typeof payload.error==="string"?payload.error:`HTTP ${response.status}`}`);continue}
   const offers=(payload.result?.rates??[]).filter(item=>Number.isFinite(Number(item.rate))&&Number(item.rate)>0),booking=offers.find(item=>`${item.code??""} ${item.name??""}`.toLowerCase().includes("booking")),selected=booking??offers.sort((a,b)=>Number(a.rate)-Number(b.rate))[0];
   if(!selected){attemptErrors.push(`${checkIn}: no OTA rate returned`);continue}
   return {hotel:competitor.name,status:days===0?"AVAILABLE":"FALLBACK_DATE",rate:Number(selected.rate),currency:criteria.currency,requestedCheckIn:criteria.checkIn,availableCheckIn:checkIn,availableCheckOut:checkOut,daysShifted:days,datesChecked:`${criteria.checkIn} to ${checkIn}`,room:"Lowest available double-occupancy room",mealPlan:"Not specified",cancellation:"Check provider terms",taxes:"Check provider terms",source:`${selected.name||"OTA"} via Xotelo`,sourceUrl:competitor.url||safeGoogleTravelLink(competitor.name,checkIn,checkOut,criteria.currency),note:`Date-specific Xotelo rate for 1 room and 2 adults${booking?"; Booking.com offer selected":"; lowest returned OTA offer selected"}.`}
  }catch(error){attemptErrors.push(`${checkIn}: ${error instanceof Error?error.message:"request failed"}`)}
 }
 return emptyResult(competitor.name,criteria,`Xotelo: ${attemptErrors.slice(-3).join(" · ")||"no rate returned"}`,competitor.url||safeGoogleTravelLink(competitor.name,criteria.checkIn,criteria.checkOut,criteria.currency));
}
...
async function searchCompetitor(apiKey:string,competitor:Competitor,criteria:Criteria,searchArea:string):Promise<RateResult>{
 const nights=stayLength(criteria.checkIn,criteria.checkOut),attemptErrors:string[]=[];
 for(let days=0;days<=7;days++){
  const checkIn=addDays(criteria.checkIn,days),checkOut=addDays(checkIn,nights),publicLink=safeGoogleTravelLink(competitor.name,checkIn,checkOut,criteria.currency),queries=[`${competitor.name} ${searchArea||"Sri Lanka"} hotel`];
  try{
...
export async function POST(request:NextRequest){
 try{
  ...
  const savedData=savedProfile?.profile_data&&typeof savedProfile.profile_data==="object"?savedProfile.profile_data as Record<string,unknown>:{},savedAnalysis=savedData.competitorRateAnalysis&&typeof savedData.competitorRateAnalysis==="object"?savedData.competitorRateAnalysis as Record<string,unknown>:null,savedResults=savedAnalysis&&Array.isArray(savedAnalysis.results)?savedAnalysis.results as Array<Record<string,unknown>>:[];
  const cachedFallback=(reason:string)=>NextResponse.json({ok:true,fallback:true,message:savedAnalysis?"Live rate provider limit reached. Last saved rates are still displayed; use Edit Rates for current changes.":"Live rate provider limit reached. Use Edit Rates to add today’s prices.",analysis:savedAnalysis?{...savedAnalysis,fallbackMode:"CACHED",lastAttemptAt:new Date().toISOString(),fallbackReason:reason}:null});
  const serpApiKey=process.env.SERPAPI_API_KEY,searchArea=property.location||"Sri Lanka",ownXotelo=savedData.xotelo as {hotelKey?:string;matchedName?:string}|undefined;
  const searchWithFallback=async(competitor:Competitor)=>{const xotelo=await searchXotelo(competitor,criteria);if(xotelo.rate!=null)return xotelo;if(!serpApiKey)return xotelo;const serp=await searchCompetitor(serpApiKey,competitor,criteria,searchArea);return serp.rate!=null?serp:{...serp,note:`${xotelo.note} · ${serp.note}`}};
  if(!serpApiKey&&!ownXotelo?.hotelKey&&!competitors.some(item=>item.xoteloHotelKey))return cachedFallback("No matched Xotelo properties and SERPAPI_API_KEY is not configured");
  const freshOurRate=await searchWithFallback({name:property.name,url:"",active:true,xoteloHotelKey:ownXotelo?.hotelKey,xoteloName:ownXotelo?.matchedName});if(providerLimit(freshOurRate))return cachedFallback(freshOurRate.note);const savedOurRate=Number(savedAnalysis?.ourRate),ourRateResult=freshOurRate.rate==null&&Number.isFinite(savedOurRate)&&savedOurRate>0?{...freshOurRate,status:"CACHED" as const,rate:savedOurRate,source:"Last saved rate",note:"Fresh rate unavailable; retained the last saved rate."}:freshOurRate,results:RateResult[]=[];for(const competitor of competitors){const fresh=await searchWithFallback(competitor);if(providerLimit(fresh))return cachedFallback(fresh.note);const saved=savedResults.find(item=>String(item.hotel??"").trim().toLowerCase()===competitor.name.trim().toLowerCase()),savedRate=Number(saved?.rate);results.push(fresh.rate==null&&Number.isFinite(savedRate)&&savedRate>0?{...fresh,status:"CACHED",rate:savedRate,source:"Last saved rate",sourceUrl:String(saved?.sourceUrl??fresh.sourceUrl),note:"Fresh rate unavailable; retained the last saved rate."}:fresh)}
  const exactRates=results.filter(result=>result.rate!=null&&result.daysShifted===0).map(result=>result.rate as number).sort((a,b)=>a-b),allRates=results.filter(result=>result.rate!=null).map(result=>result.rate as number).sort((a,b)=>a-b),marketRates=exactRates.length>=2?exactRates:allRates,marketMedian=marketRates.length?marketRates[Math.floor(marketRates.length/2)]:null,occupancy=Number(snapshot?.occupancy_percent??0),liveOurRate=ourRateResult.rate??null,occupancyFactor=occupancy>=90?1.15:occupancy>=76?1.10:occupancy>=56?1.05:occupancy>0&&occupancy<=30?.94:occupancy>30&&occupancy<=45?.98:1,marketTarget=liveOurRate==null?null:marketMedian==null?liveOurRate*occupancyFactor:marketMedian*occupancyFactor,lowerLimit=liveOurRate==null?null:liveOurRate*.88,upperLimit=liveOurRate==null?null:liveOurRate*1.12,suggestedRate=marketTarget==null?null:Math.round(lowerLimit==null||upperLimit==null?marketTarget:Math.min(upperLimit,Math.max(lowerLimit,marketTarget))),changePercent=liveOurRate&&suggestedRate!=null?Math.round((suggestedRate-liveOurRate)/liveOurRate*100):null;
  ...
  await db.from("intelligence_runs").insert({module:"YIELD",run_type:user.scheduled?"SCHEDULED":"MANUAL",status:"COMPLETED",requested_by:user.id,started_at:new Date().toISOString(),completed_at:new Date().toISOString(),result_summary:{propertyId:property.id,checkIn:criteria.checkIn,checkOut:criteria.checkOut,competitorCount:competitors.length,verifiedRateCount:verifiedCount,fallbackCount,advisoryOnly:true}});
