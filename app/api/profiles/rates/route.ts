...
export async function POST(request:NextRequest){
 try{
  ...
  const savedData=savedProfile?.profile_data&&typeof savedProfile.profile_data==="object"?savedProfile.profile_data as Record<string,unknown>:{},profilePayload:Record<string,unknown>={property_id:property.id,profile_data:{...savedData,competitorRatePreviousAnalysis:savedData.competitorRateAnalysis??null,competitorRateAnalysis:analysis},updated_by:user.id,updated_at:new Date().toISOString()};if(savedProfile?.id)profilePayload.id=savedProfile.id;
