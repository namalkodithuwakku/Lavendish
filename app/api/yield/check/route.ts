        ...
        if (!previous) {
          ...
          if (changed) {
            if (fullStateChanged) {
              ...
              if (categoryChanged&&await createAlert(db,profile,settings,previous,current))createdAlerts++;
              if (await createInventoryAlert(db,profile,previous,current))createdAlerts++;
            }
          }
        }
