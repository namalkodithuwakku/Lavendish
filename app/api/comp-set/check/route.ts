...
async function runChecks(request:NextRequest){
 ...
 const checkIn=dateInColombo(),checkOut=dateInColombo(1),skipped:Array<{hotel:string;ok:false;message:string;competitorCount:0}>=[],jobs=(data??[]).flatMap(property=>{const relation=property.property_profiles as unknown,profiles=Array.isArray(relation)?relation:relation?[relation]:[],profile=profiles[0] as {profile_data?:Record<string,unknown>}|undefined,profileData=profile?.profile_data??{},set=profileData.competitorSet as {main?:Array<{name:string;url:string;active:boolean}>;additional?:Array<{name:string;url:string;active:boolean}>;criteria?:Record<string,string>}|undefined,seen=new Set<string>(),competitors=[...(set?.main??[]),...(set?.additional??[])].filter(item=>{const key=item.name?.trim().toLowerCase();if(!item.active||!key||seen.has(key))return false;seen.add(key);return true}).slice(0,7);if(!competitors.length){skipped.push({hotel:property.name,ok:false,message:"No active competitors saved in Hotel Profiles",competitorCount:0});return []}return [{propertyId:property.id,name:property.name,competitors,criteria:{...(set?.criteria??{}),checkIn,checkOut,rooms:"1",adults:"2",children:"0",childAges:"",roomType:"Lowest available double occupancy",currency:set?.criteria?.currency||"USD",source:"Booking.com"}}]});
 const completed=await Promise.all(jobs.map(async job=>{try{const response=await fetch(new URL("/api/profiles/rates",request.nextUrl.origin),{method:"POST",headers:{"Content-Type":"application/json",Authorization:request.headers.get("authorization")??""},body:JSON.stringify(job),cache:"no-store"}),payload=await response.json() as {error?:string};return {hotel:job.name,ok:response.ok,message:payload.error,competitorCount:job.competitors.length}}catch(error){return {hotel:job.name,ok:false,message:error instanceof Error?error.message:"Check failed",competitorCount:job.competitors.length}}}));
 const results=[...completed,...skipped],succeeded=completed.filter(item=>item.ok).length,failed=results.filter(item=>!item.ok).length;
 return {ok:failed===0,checkedAt:new Date().toISOString(),schedule:"06:00, 11:00, 15:00 Asia/Colombo",hotels:(data??[]).length,attempted:jobs.length,skipped:skipped.length,succeeded,failed,results};
}
