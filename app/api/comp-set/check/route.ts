...
function authorized(request:NextRequest){const header=request.headers.get("authorization")??"",secrets=[process.env.CRON_SECRET,process.env.YIELD_CRON_SECRET].filter(Boolean);return secrets.some(secret=>header===`Bearer ${secret}`)}
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const results:Array<{hotel:string;ok:boolean;message?:string}>=[];
  let cursor=0;async function worker(){while(cursor<jobs.length){const job=jobs[cursor++];try{const response=await fetch(new URL("/api/profiles/rates",request.nextUrl.origin),{method:"POST",headers:{"Content-Type":"application/json",Authorization:request.headers.get("authorization")??""},body:JSON.stringify(job),cache:"no-store"}),payload=await response.json() as {error?:string};results.push({hotel:job.name,ok:response.ok,message:payload.error})}catch(error){results.push({hotel:job.name,ok:false,message:error instanceof Error?error.message:"Check failed"})}}}
  await Promise.all(Array.from({length:Math.min(2,jobs.length)},()=>worker()));
  return NextResponse.json({ok:results.every(item=>item.ok),checkedAt:new Date().toISOString(),schedule:"06:00, 11:00, 15:00 Asia/Colombo",hotels:results.length,results});
 }catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Scheduled Comp Set check failed"},{status:500})}
}
