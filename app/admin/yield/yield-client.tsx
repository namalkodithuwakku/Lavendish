...
export default function YieldClient(){
  ...
  return <main className="admin-shell">
    <aside className="admin-nav"><div className="admin-brand"><span>NK</span><div><b>N K Hotels</b><small>Lavendish Management</small></div></div><nav><Link href="/">⌂ <span>Occupancy dashboard</span></Link><Link href="/admin">▦ <span>Hotel profiles</span></Link><Link href="/admin/users">♙ <span>Users & access</span></Link><Link className="active" href="/admin/yield">↗ <span>Yield rules</span></Link><Link href="/alerts/ota">◉ <span>OTA alerts</span></Link><Link href="/alerts/yield">⌁ <span>Yield alerts</span></Link></nav><div className="admin-identity"><span>NK</span><div><b>{session?.user.user_metadata?.full_name??"Master"}</b><small>Master Admin</small></div></div></aside>
    <section className="admin-page yield-page">
      ...
      <section className="architecture-note"><span>✓</span><div><b>Safe workflow</b><p>The engine recommends rates and creates followable alerts for authorised staff. It never changes Google Sheets or OTA extranets automatically. Duplicate active alerts for the same hotel, date and threshold are blocked.</p></div></section>
    </section>
