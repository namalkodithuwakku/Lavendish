...
const TYPE_LABELS:Record<string,string>={RATE_UPDATE:"Rate update",OTA_CLOSURE:"OTA closure",OTA_REOPEN:"OTA reopen",OCCUPANCY:"Occupancy"};
...
export default function AlertsClient({mode}:{mode:Mode}){
  ...
  const [alerts,setAlerts]=useState<Alert[]>([]),[loading,setLoading]=useState(true),[notice,setNotice]=useState(""),[hotel,setHotel]=useState("ALL"),[status,setStatus]=useState(mode==="ota"?"OPEN":"ALL"),[updating,setUpdating]=useState("");
  ...
  const pageAlerts=useMemo(()=>alerts.filter(alert=>mode==="yield"||["RATE_UPDATE","OTA_CLOSURE","OTA_REOPEN"].includes(alert.alert_type)),[alerts,mode]);
  ...
  return <main className="alerts-shell">
    ...
    <section className="alerts-page">
      <header className="alerts-title"><div><p>{mode==="ota"?"OPERATIONS QUEUE":"YIELD MANAGEMENT"}</p><h1>{mode==="ota"?"OTA update alerts":"Yield management alerts"}</h1><span>{mode==="ota"?"Follow rate changes, OTA closures and reopening actions without creating tasks.":"Review occupancy movements, available rooms, thresholds and suggested rates."}</span></div><button onClick={()=>void load()} disabled={loading}>{loading?"Refreshing…":"Refresh alerts"}</button></header>
      ...
      <section className="alerts-list">
        {filtered.map(alert=><article className={`alert-card ${alert.alert_type.toLowerCase()} ${alert.status.toLowerCase()}`} key={alert.id}>
          ...
          <div className="alert-rates"><header><div><b>Suggested rates</b><small>Based on current rooms sold and active plans</small></div>{alert.threshold&&<span>{alert.threshold}% level</span>}</header>{alert.suggested_rates?.length?<div>{alert.suggested_rates.map(rate=><article key={rate.planCode}><span>{rate.planName}</span><b>{rate.currency} {Number(rate.rate).toLocaleString()}</b><small>Sold band {rate.soldFrom}–{rate.soldTo}</small></article>)}</div>:<p>Rate rule required for this occupancy level.</p>}</div>
